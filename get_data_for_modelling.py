...
def main():
    ...
    scores=pd.read_csv("historic_scores.csv")
    adjusted_scores = adjust_fields(scores)
    scores_sliding = sliding_filling_for_model(adjusted_scores)
    complete_scores = fill_cuotas(scores_sliding)
    complete_scores.to_csv("scores_enhanced.csv",index=False)
